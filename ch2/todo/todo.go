...
// COmpelte method marks a ToDo item as completed by setting Done = true and
// CompletedAt to the current time
func (l *List) Complete(i int) error {
	// Technically it does not need to use a pointer reciever but it's a good
	// practice to keep the method set consistent
	ls := *l
	if i <= 0 || i > len(ls) {
