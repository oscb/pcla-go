...
func createTempDir(t *testing.T, files map[string]int) (dirname string, cleanup func()) {
	t.Helper() // Declare as a test helper for correct test logging
...
func TestRunDelExtension(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Setup test directory and defer cleanup
			...
			if len(lines) != expLogLines {
				t.Errorf("Expected %d log lines, go %d instead\n", expLogLines, len(lines))
			}
		})
	}
}
...
func TestRunArchive(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Get the expected files by running a glob matcher over the temp files
			...
			res := strings.TrimSpace(buffer.String()) // Remove the last new line on the output
			if expOut != res {
